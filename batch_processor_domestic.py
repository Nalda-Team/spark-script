...
def process_domestic_batch(spark, file_paths):
    ...
    # 국내선 항공편 처리
    flight_info_df, fare_info_df = process_domestic_flights_df(df, airport_map_bc)
    
    if flight_info_df is not None and fare_info_df is not None:
        print("국내선 항공권 데이터 처리 완료")
        
        # 중복 제거 및 파티션 최적화
        flight_info_df = optimize_partitions(flight_info_df, ["air_id"])
        print('flight_info_df 파티셔닝 완료')
        # fare_info는 복합 키로 중복 제거 
        fare_info_df = optimize_partitions(fare_info_df, ["air_id", "seat_class", "agt_code", "fetched_date", "fare_class"])
        print('fare_info_df 파티셔닝 완료')
...
def process_folder_in_batches(bucket_name, folder_path, batch_size=10, file_type='domestic'):
    ...
    for i, batch in enumerate(batches):
        ...
        process_domestic_batch(spark, batch)
